<template>
    ...
    <v-tabs-window v-model="tab.value">
        ...
        <v-tabs-window-item value="container">
            ...
            <v-data-table :loading="tableContainers.loading" :headers="tableContainers.headers" items-per-page="10"
                ...
                <template v-slot:item.status="{ item }">
                    <v-chip v-if="['running', 'active'].indexOf(item.status) >= 0" color="success">{{ item.status
                    }}</v-chip>
                    <v-chip v-else color="red">{{ item.status }}</v-chip>
                </template>
                <template v-slot:item.actions="{ item }">
                    <v-btn color="info" size='small' variant='text' @click="() => { }">元数据</v-btn>
                    <v-btn v-if="['exited', 'stopped'].indexOf(item.status) >= 0" color="success" size='small' variant='text'
                        @click="tableContainers.startContainer(item)">启动</v-btn>
                    <v-btn v-else-if="['running', 'active'].indexOf(item.status) >= 0" color="warning" size='small' variant='text'
                        @click="tableContainers.stopContainer(item)">停止</v-btn>
        ...
        <v-tabs-window-item value="image">
            ...
            <v-row class="mt-1">
                <v-col cols="4">
                    <v-text-field v-model="tableImages.search" placeholder="搜索" flat variant="outlined" class="mb-4"
                        prepend-inner-icon="mdi-magnify" />
                </v-col>
                <v-col class='ma-0'>
...
var tab = reactive({ value: 'overview' })
